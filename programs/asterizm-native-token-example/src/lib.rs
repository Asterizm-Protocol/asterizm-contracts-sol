...
#[program]
mod asterizm_native_token_example {
    use super::*;
    ...
    pub fn send_message(
        ...
        system_transfer(cpi_context, fee)?;
        ...
        transfer(ctx.accounts.to_transfer_cpi(signer), amount)?;
    ...
    pub fn receive_message(
        ...
        let seeds: &[&[_]] = &[
            &ctx.accounts.token_client_account.authority.to_bytes(),
            _name.as_bytes(),
            b"asterizm-token-vault",
            &[ctx.accounts.token_client_account.vault_bump],
        ];
        let signer: &[&[&[u8]]] = &[&seeds[..]];

        transfer(ctx.accounts.to_transfer_cpi(signer), data.amount)
    }
    ...
    pub fn process_refund_request(
        ...
        if status {
            ...
            system_transfer(cpi_context, refund_fee)?;
            ...
            transfer(
                ctx.accounts.to_transfer_cpi(signer),
    ...
    pub fn send_from_vault(ctx: Context<SendFromVault>, name: String, amount: u64) -> Result<()> {
        let seeds: &[&[_]] = &[
            &ctx.accounts.token_client_account.authority.to_bytes(),
            name.as_bytes(),
            b"asterizm-token-vault",
    ...
    pub fn create_vault(
        ...
        ctx.accounts.token_client_account.vault_bump = ctx.bumps.vault;
...
pub struct SendMessage<'info> {
    ...
    #[account(mut,
        ...
        seeds = [token_authority.key().as_ref(), _name.as_bytes(), b"asterizm-token-vault"],
        bump
    )]
...
pub struct ProcessRefundRequest<'info> {
    ...
    #[account(mut,
        token::mint = mint,
        token::authority = vault,
        seeds = [&token_client_account.authority.to_bytes(), _name.as_bytes(), b"asterizm-token-vault"],
        bump
    )]
    pub vault: Account<'info, TokenAccount>,
    /// CHECK: This is not dangerous because we will check it inside client
...
pub struct SendFromVault<'info> {
    ...
    #[account(mut,
        token::mint = mint,
        token::authority = vault,
        seeds = [authority.key().as_ref(), name.as_bytes(), b"asterizm-token-vault"],
        bump
    )]
    pub vault: Account<'info, TokenAccount>,
    pub token_program: Program<'info, Token>,
...
impl<'a, 'b, 'c, 'info> CreateVault<'info> {
...
impl<'a, 'b, 'c, 'info> SendFromVault<'info> {
    fn to_transfer_cpi(
        &self,
        seeds: &'a [&'b [&'c [u8]]],
    ) -> CpiContext<'a, 'b, 'c, 'info, anchor_spl::token::Transfer<'info>> {
